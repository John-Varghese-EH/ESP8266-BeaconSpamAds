...
void CaptivePortal::setup() {
  ...
  WiFi.softAP("Connect me!"); // We could make this configurable too if we wanted
...
void CaptivePortal::handleNotFound() {
  // If trying to access admin IPs directly, let it through? No, safer to just redirect everything unless it matches specific routes
...
void CaptivePortal::handleGetData() {
  ...
  json += "\"appendSpaces\":" + String(storage.config.appendSpaces ? "true" : "false") + ",";
  ...
  for(size_t i=0; i<storage.ssids.size(); i++) {
    ...
    json += s;
    if(i < storage.ssids.size() - 1) json += "\\n";
  }
  json += "\"";
...
void CaptivePortal::handleSaveConfig() {
  ...
  if(intervalIdx > 0) {
     ...
     if(endIdx < 0) endIdx = data.indexOf(",", intervalIdx);
...
void CaptivePortal::handleSaveSSIDs() {
  ...
  server.send(200, "text/plain", "SSIDs Saved (" + String(storage.ssids.size()) + ")");
}
