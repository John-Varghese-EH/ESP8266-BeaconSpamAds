...
void BeaconSpam::update() {
  ...
  if (currentTime - attackTime > storage.config.beaconInterval) {
    ...
    if (storage.ssids.empty()) return; // Safety check

    // Cycle through SSIDs
    static size_t ssidIndex = 0;
    if (ssidIndex >= storage.ssids.size()) ssidIndex = 0;
    
    // We try to send as many as possible within a small window, or just one per interval
    // Original code tried to send all, but that blocks loop and webserver. 
    // Let's send a batch (e.g., 3-5 SSIDs) per update to balance spam vs webserver responsiveness
    ...
    for(int b=0; b<batchSize; b++) {
        ...
        if(parserSsidLen > 32) parserSsidLen = 32;

        // static int macOffset = 0;
        // macAddr[5] = (uint8_t)(macOffset++); // Change MAC slightly for each SSID so they show up as diff APs
        // Actually, let's just randomize the last byte based on index to keep it stable-ish per SSID
        ...
        if (storage.config.appendSpaces) {
            ...
            memcpy(&tmpPacket[0], &beaconPacket[0], 38 + parserSsidLen); // copy first half of packet into buffer
            tmpPacket[37] = parserSsidLen; // update SSID length byte
            memcpy(&tmpPacket[38 + parserSsidLen], &beaconPacket[70], storage.config.wpa2 ? 39 : 13); // copy second half
